...
"""
# set filename
my_filename = (r'f1.rat4.25.cf.rf_1(1).iox.xlsx')
# my_list = glob.glob(my_filename)
# boolean to detect empty rows
empty_rows = False  # boolean to detect empty rows
save = True # boolean to save excel
...
def mean_bin(df: pd.DataFrame, baseline: int, bin_length: int, measures: list):
    ...
    for i in range(len(period_start_index)):  # iterate over period_start entries
        ...
        if comment_index.empty:  # if there are no comments, discard this period_start/period_stop section
            ...
            for l in range(len(comment_index) - 1):
                injection_df = period_df.loc[comment_index[l]+1:comment_index[l+1]-1, :]
                for k in range(comment_index[l]+1, comment_index[l+1], window_size):
                    print(f'Bin: {key}')
                    ...
                    key += bin_length  # update bin number
                print(f'Finished injection {l+1}')

            injection_df = period_df.loc[comment_index[-1]+1:period_stop_index[i], :]
            for k in range(comment_index[-1]+1, period_stop_index[i], window_size):
                print(f'Bin {key}')
                print(injection_df.loc[k:k+window_size, :])
                injection_mean = injection_df.loc[k:k+window_size, measures].mean(skipna=True)
                bin_df[key] = injection_mean
                bin_df = bin_df.copy()
                key += bin_length
            print(f'Finished injection {len(comment_index)}')
...
cleaned_df_columns = ['cpu-date', 'cpu-time', 'site-time', 'period-time', 'run-events', 'comments'] + relevant_measures
clean_name = splitext + '_cleaned.xlsx'
# print(cleaned_df.to_string())
cleaned_df.loc[:, cleaned_df_columns].to_excel(clean_name)
...
if plot:
    ...
    for index in my_bin_df.index[5:]:  # iterate over measures
        baseline_average = my_bin_df.loc[index, -my_baseline:-5].mean()  # calculate the baseline average
