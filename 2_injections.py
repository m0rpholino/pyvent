...
"""
# set filename
my_filename = (r'analyses\july 2\*.xlsx')
...
plot = True  # boolean to show plots
        ...
        if comment_index.empty:  # if there are no comments, discard this period_start/period_stop section
            ...
            for l in range(len(comment_index) - 1):
                ...
                for k in range(comment_index[l]+1, comment_index[l+1], window_size):
                    if comment_index[l+1]-1 - k < 2*window_size:
                        print(f'Bin:{key}')
                        print(injection_df.loc[k:comment_index[l+1]-1])
                        injection_mean = injection_df.loc[k:comment_index[l+1]-1, measures].mean(skipna=True)
                        bin_df[key] = injection_mean
                        bin_df = bin_df.copy()
                        key += bin_length
                        print(f'Finished injection {i+1}')
                        break
